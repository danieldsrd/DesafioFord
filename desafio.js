...
const capitaisSudeste = ['Rio de Janeiro', 'São Paulo', 'Belo Horizonte', 'Vitória']; 
  ...
  for (index = 0; index < newarray.length; index++) {
    for (j = 0; j < rotas.length; j++) {
      if (rotas[j] !== newarray[index]) {
        equal.indexOf(newarray[index]) === -1 ? equal.push(newarray[index]) : console.log();
      }
    }
  }
  console.log(equal);
